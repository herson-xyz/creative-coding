...
async function main() {
  ...
  const jitterBuffer = gpu.createBuffer({
    size: sizes.u32,
    usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.UNIFORM,
  });
  gpu.queue.writeBuffer(jitterBuffer, 0, new Uint32Array([uniforms.jitter]));
  ...
  /////////////////////////
  // UI controls
  const jitterBtn = document.getElementById("jitter-toggle");
  jitterBtn.addEventListener("click", () => {
    uniforms.jitter = uniforms.jitter === 1 ? 0 : 1;
    jitterBtn.textContent = uniforms.jitter ? "Jitter: ON" : "Jitter: OFF";
    jitterBtn.dataset.active = uniforms.jitter;
    gpu.queue.writeBuffer(jitterBuffer, 0, new Uint32Array([uniforms.jitter]));
  });
